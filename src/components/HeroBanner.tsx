...
interface HeroBannerProps {
  scrollToRef?: RefObject<HTMLDivElement>; // ⭐️ prop التمرير
}

const HeroBanner = ({ scrollToRef }: HeroBannerProps) => {
  const handleScroll = () => {
    if (scrollToRef?.current) {
      scrollToRef.current.scrollIntoView({ behavior: "smooth" }); // ⭐️ التمرير السلس
    }
  };
        ...
        <div className="max-w-4xl mx-auto">
          ...
          <div className="flex flex-col sm:flex-row gap-3 justify-center animate-slide-up px-4" style={{ animationDelay: "0.6s" }}>
            ...
            <Button
              size="lg"
              variant="outline"
              onClick={handleScroll} // ⭐️ عند الضغط يتم التمرير
